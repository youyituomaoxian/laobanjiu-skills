# Adversarial Review — 对抗性代码审查 Skill

在 WorkBuddy GUI 中运行的对抗性代码审查流水线。Red Team 攻击 → Blue Team 辩护 → 主 Agent 裁决 → 自动修复 → QA 验证。一句话触发，六阶段闭环。

## 安装

```bash
git clone https://github.com/YOUR_USER/adversarial-review.git ~/.workbuddy/skills/adversarial-review
```

或下载 zip → 解压到 `~/.workbuddy/skills/adversarial-review/`。
...
review src/utils/api.js adversarial
```
...
```
adversarial-review/
├── SKILL.md                     # 完整 6 阶段流程 + 角色定义
├── README.md                    # 本文档（设计说明 + 对比分析）
├── references/
│   └── attack_vectors.md        # 攻击线索库（通用 + JS/React/Python）
└── assets/
...
所有审查中间产物写入 `<workspace>/.workbuddy/review/`：

| 文件 | 阶段 |
|------|------|
| `00_preflight.md` | lint/build 预检 |
| `01_red.md` | Red Team 攻击报告 |
| `02_blue.md` | Blue Team 辩护报告 |
| `03_verdict.md` | 裁决报告 |
| `04_changes.md` | 修复 diff |
| `05_final.md` | QA 验证 + GO/NO-GO |
